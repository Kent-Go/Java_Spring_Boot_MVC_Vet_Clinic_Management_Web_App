...
public class PetOwnerProfileControllerTest {
    ...
    public void testShowPetOwnerProfile_NotFound() throws Exception {
        when(petOwnerService.getPetOwnerByUserID(1)).thenReturn(null);

        mockMvc.perform(get("/petOwnerProfile").param("petOwnerId", "1"))
                .andExpect(status().isNotFound());

        verify(petOwnerService, times(1)).getPetOwnerByUserID(1);
    }
    ...
    public void testShowPetOwnerProfile_InvalidId() throws Exception {
        // Perform GET request with an invalid ID (negative value) and expect a 400 Bad Request
        mockMvc.perform(get("/petOwnerProfile").param("petOwnerId", "-1"))
    ...
    public void testUpdatePetOwnerProfile_Success() throws Exception {
        // Mock objects
        User mockUser = new User("John", "Doe", LocalDate.of(1990, 1, 1), "Male", "1234567890", "john@example.com", "password");
