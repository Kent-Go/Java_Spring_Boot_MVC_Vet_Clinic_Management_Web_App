...
class LoginControllerTest {
    ...
    // Test processing a login request with an email that has more than 1 special
    // character
    @Test
    ...
    void testProcessLoginWithValidCredentialsForPetOwner() throws Exception {
        ...
        // Mock the behavior of UserService to return this user when queried by email
        when(userService.getUserByEmail("validuser@example.com")).thenReturn(user);
        ...
        mockMvc.perform(post("/login")
                ...
                .andExpect(redirectedUrl("/petOwnerWelcome?userId=1")); // Verify the redirection URL
    }
    ...
    void testProcessLoginWithValidCredentialsForVet() throws Exception {
        ...
        mockMvc.perform(post("/login")
                ...
                .param("password", "password123"))
                .andExpect(status().is3xxRedirection()) // Expect a redirect status (successful login)
                .andExpect(redirectedUrl("/vetDashboard?userId=1&vetId=1")); // Verify the redirection URL
    }
